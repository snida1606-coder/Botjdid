...
def auto_trade_loop(trader, context):
    ...
    async def run():
        ...
        while trader.running:
          ...
          # session (and each user runs on its OWN thread/loop/client → isolated)
          try:
            ...
            if outcome != "loss":
                ...
                if outcome == "win":
                    trader.win_count += 1
                    trader.win_streak += 1; trader.loss_streak = 0
...
def _auto_control_keyboard(trader):
    ...
    return InlineKeyboardMarkup([[toggle,
                                  InlineKeyboardButton("📊 Status", callback_data="atx_status")],
                                 [InlineKeyboardButton("🔴 Stop", callback_data="atx_stop")]])
...
def _auto_report_card(trader, title="DAILY REPORT"):
    ...
    won, lost, tie = trader.win_count, trader.loss_count, getattr(trader, "tie_count", 0)
    decided = won + lost
    wr = (won / decided * 100) if decided else 0.0
    pnl = _auto_session_pnl(trader)
...
async def auto_start_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    t = threading.Thread(target=auto_trade_loop, args=(trader, context), daemon=True)
    trader._thread = t
    t.start()
